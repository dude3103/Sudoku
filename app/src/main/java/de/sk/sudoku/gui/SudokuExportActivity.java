...
import de.sk.sudoku.gui.exporting.FileExportTask;
import de.sk.sudoku.gui.exporting.FileExportTaskParams;
import de.sk.sudoku.gui.exporting.FileExportTaskResult;
import de.sk.sudoku.gui.exporting.FileExportTask.OnExportFinishedListener;
...
//			mExportParams.sudokuID = intent.getLongExtra(EXTRA_SUDOKU_ID, 0);
	...
	private void exportToFile() {
		...
		File file = new File(directory, filename + ".sudoku");
		if (file.exists()) {
	...
	private void startExportToFileTask() {
		...
		mExportParams.file = new File(directory, filename + ".sudoku");
...
//					intent.setType(Const.MIME_TYPE_SUDOKU);
